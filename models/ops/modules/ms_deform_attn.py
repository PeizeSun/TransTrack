...
class MSDeformAttn_pytorch_mot(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, im2col_step=64):
        super().__init__()
        assert d_model % n_heads == 0
...
#         output = MSDeformAttnFunction.apply(
#             value, input_spatial_shapes, sampling_locations, attention_weights, self.im2col_step)
        output = ms_deform_attn_core_pytorch_mot(
            self.query_proj(query), value, input_spatial_shapes, sampling_locations, self.key_proj)
